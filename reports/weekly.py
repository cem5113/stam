...
def make_weekly(df: pd.DataFrame,
    ...
    """
    Haftalık rapor: end dâhil son 7 gün (end-6 .. end).
    Dönenler:
...
def export_weekly_zip(df: pd.DataFrame,
    ...
    """
    Haftalık raporu ZIP’e yazar ve dosya yolunu string olarak döndürür.
    ZIP adı: weekly_YYYY-MM-DD_YYYY-MM-DD_*.zip
    """
    end_norm = pd.to_datetime(end or _infer_end(df)).normalize()
    d1 = (end_norm - pd.Timedelta(days=6)).normalize()
    d2 = end_norm

    tables, tips = make_weekly(df, end=end_norm, cats=cats)
    prefix = f"weekly_{d1.date()}_{d2.date()}"
